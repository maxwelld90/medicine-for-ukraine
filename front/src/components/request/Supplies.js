      ...
      {() => (
        <div>
          ...
          <ul className="item-list items direction">
            {value.map((product) => (
              <li key={product.names.default} className={getProductClasses(product)} onClick={() => selectProduct(product)}>
                <span className="name">{product.names[language] || product.names.default}</span>
                <span className="right-background" />
                {product.highPriority && <span className="high-priority" />}
                {product.lowestPrice && (
                  <>
          ...
          <StepNavigation prevButtonTitle={t('common:PREV_BUTTON')} onClickPrev={onBack} />
        </div>
      )}
    </Content>
  )
